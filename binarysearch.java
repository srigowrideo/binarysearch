...
public class binarysearch {
		...
		int length = commandReader.nextInt(); int[] input = new int[length]; 
		System.out.printf("Enter %d integers %n", length); for (int i = 0; i < length; i++) 
...
public static int performBinarySearch(int[] input, int number) {
	...
	{ return middle; } else if (input[middle] < number) { low = middle + 1; } 
	else if (input[middle] > number) { high = middle - 1; } } return -1; } 
}
